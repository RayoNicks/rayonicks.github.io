...
### 19.6.2 侦测任意成员类型 {#Detecting-Arbitrary-Member-Types}

为每个成员类型都定义上述的模板过于麻烦了，可以使用宏来简化：
...
类型特征模板起作用的原因是定义了成员类型`Type`，所以不得不显示的写出`::Type`来获取类型特征，为了简便可以使用别名模板来代替：
