...
下面分析下构造函数的重载解析过程。1和2参数数量不匹配，3中`Head`为`long`，不能匹配`Tuple<int, double, std::string>`。根据16章中模板特化的偏序规则，得到4的参数列表为`(A1&&)`，5的参数列表为`(Tuple<A2, A3...> const&)`，4的推导结果`Tuple<int, double, std::string>&`需要增加`const`才能和5的参数列表完全匹配，而5的推导结果`Tuple<int, double, std::string> const&`可以和4的参数列表完全匹配（只需要将`A1`替换为5的推导结果即可），因此4比5更为特化，编译器将匹配构造函数4，这就导致了在构造`head`的过程中出现类型转换的错误。
...
类似标准库中的`std::make_tuple()`，还可以通过`makeTuple()`构造元组：
